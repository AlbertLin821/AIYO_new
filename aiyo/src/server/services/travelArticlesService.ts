...
async function fetchDcardPostsViaWebSearch(
  ...
  if (collected.length < limit && serverConfig.searxngEnabled) {
    const searxResults = await searchWeb({ query: searchQuery, limit: fetchLimit });
    collected.push(...(await collectDcardPostsFromSearchResults(searxResults, fetchLimit)));
  }

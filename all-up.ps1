...
Write-Host "Recreating shared services and dev frontend using aiyo/.env.dev."
...
docker compose @devComposeEnvArgs up -d --build --force-recreate @sharedServices
if ($LASTEXITCODE -ne 0) {
    exit $LASTEXITCODE
}

Write-Host ""
Write-Host "Recreating prod-live frontend using aiyo/.env.prod-live."
